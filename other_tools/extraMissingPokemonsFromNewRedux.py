...
final_abilities : list[Ability] = [Ability.model_validate(p) for p in raw_final_abilities]
with open("../output/finals/moves.json","r") as f:
    raw_final_moves = json.load(f)
final_moves : list[FullMove] = [FullMove.model_validate(p) for p in raw_final_moves]
new_pokemons = []
redux_mons = []
...
for mon in data["species"]:
    if mon["name"] != "??????????":
        ...
        for move_oj in mon["levelUpMoves"]:
            ...
            move_exist = list(filter(lambda x: x.id != -1 and x.id == move_id, final_moves))
            if len(move_exist) > 0:
