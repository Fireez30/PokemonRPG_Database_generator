...
input_redux = "/home/benjamin/Projects/Others/FaeresDevWebsite/server/pokemon_data/pokemon.json"

with open(input_redux, "r") as f:
    poke_redux = json.load(f)
...
target = "scorpeon"
find_target = False
for poke in poke_redux:
    poke_name = poke["name"]
    if poke_name.lower() == "scorpeon":
        find_target = True
        continue
    if not find_target:
        continue
    if not "florges" in poke_name.lower() and not "floette" in poke_name.lower() and not "flabebe" in poke_name.lower() and not "minior" in poke_name.lower() and not "alcremie" in poke_name.lower() and not "silvally" in poke_name.lower() and not "unown" in poke_name.lower() and not "mega" in poke_name.lower() and not "pikachu" in poke_name.lower() and not "vivillon" in poke_name.lower():
       print(poke_name)
       finals.append(poke)
...
for pdf in pdfs:
    if pdf != "../output/pdf_temp/'.pdf":
        merger.append(pdf)
merger.write("../output/pdf/redux.pdf")
merger.close()
