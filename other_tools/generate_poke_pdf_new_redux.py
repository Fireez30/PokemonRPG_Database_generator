...
if __name__ == "__main__":
    ...
    pokemon = load_pokemon("../output/finals/pokemons_redux_new.json")
    for poke in pokemon:
        create_pdf(poke, "../output/pdf_temp_redux/"+poke["name"]+".pdf")

    merger = PdfWriter()

    pdfs = list(filter(os.path.isfile, glob.glob("../output/pdf_temp_redux/*.pdf")))

    for pdf in pdfs:
        if pdf != "../output/pdf_temp_redux/'.pdf":
            merger.append(pdf)

    merger.write("../output/pdf_redux/pokedex.pdf")
    merger.close()